...
export function WalletsScreen() {
  ...
  const remove = (wallet: Wallet) => Alert.alert('Archive wallet?', `${wallet.name} will be hidden. Existing transactions keep their history.`, [{ text: 'Cancel', style: 'cancel' }, { text: 'Archive', style: 'destructive', onPress: async () => { const r = await archiveWallet(wallet.id); showToast(r.ok ? 'Wallet archived.' : r.message, r.ok ? undefined : { tone: 'warning' }); } }]);
  ...
  return <Screen bottomInset={40} variant={7} refreshing={loading} onRefresh={refresh}>
    <View style={s.page}>
      <View style={s.header}><BackButton /><View style={s.headerCopy}><AppText variant="title">Wallets</AppText><AppText style={s.muted}>Tap a card to edit it, or use ••• to add money.</AppText></View></View>
