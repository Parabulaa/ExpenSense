...
export function Brand({ compact = false }: { compact?: boolean }) { return <Image source={assets.logo} contentFit="contain" style={[styles.brand, compact && styles.brandCompact]} />; }
export function PrimaryButton({ title, onPress, icon, danger = false }: { title: string; onPress?: () => void; icon?: string; danger?: boolean }) {
  return <Pressable accessibilityRole="button" accessibilityLabel={title} onPress={onPress} style={({ pressed }) => [styles.primary, danger && { backgroundColor: colors.danger }, pressed && { opacity: .84 }]}>{icon ? <AppText variant="h2" style={{ color: colors.surface }}>{icon}</AppText> : null}<AppText variant="button" style={{ color: colors.surface }}>{title}</AppText>{!icon && <AppText variant="h2" style={styles.arrow}>→</AppText>}</Pressable>;
}
export function SecondaryButton({ title, onPress, icon }: { title: string; onPress?: () => void; icon?: string }) { return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.secondary, pressed && { opacity: .75 }]}>{icon && <AppText variant="h2" style={{ color: colors.deepForest }}>{icon}</AppText>}<AppText variant="button">{title}</AppText></Pressable>; }
export function BackButton() { return <Pressable accessibilityLabel="Go back" hitSlop={12} onPress={() => router.back()} style={styles.back}><AppText variant="h2">‹</AppText></Pressable>; }
export function FormInput({ icon, secure, right, style, ...props }: Omit<TextInputProps, 'style'> & { icon?: string; secure?: boolean; right?: ReactNode; style?: any }) { return <View style={[styles.inputWrap, style]}>{icon && <AppText variant="h3" style={styles.inputIcon}>{icon}</AppText>}<TextInput placeholderTextColor="#849096" secureTextEntry={secure} style={styles.input} {...props} />{right}</View>; }
...
const styles = StyleSheet.create({
  ...
  brand: { width: 245, height: 104 }, brandCompact: { width: 210, height: 86 }, primary: { minHeight: 54, backgroundColor: colors.deepForest, borderRadius: radii.md, paddingHorizontal: spacing.xl, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 12 },
  arrow: { position: 'absolute', right: 22, color: colors.surface }, secondary: { minHeight: 52, borderRadius: radii.md, borderWidth: 1.5, borderColor: '#C6D3C1', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 12, backgroundColor: colors.surface },
  ...
  inputIcon: { width: 32, color: colors.deepForest }, input: { flex: 1, height: 56, color: colors.text, fontFamily: 'JakartaRegular', fontSize: 15, paddingHorizontal: 8 }, card: { backgroundColor: 'rgba(255,253,247,.96)', borderRadius: radii.md, padding: spacing.lg, ...shadow },
